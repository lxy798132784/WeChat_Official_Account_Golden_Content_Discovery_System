...
WeChatSearchAutomationController::Result WeChatSearchAutomationController::dryRunPlan(const QStringList& keywords,
  ...
  if (options.searchTapX <= 0 || options.searchTapY <= 0) {
  ...
  result.commands << adbTapArguments(options.searchTapX, options.searchTapY).join(QStringLiteral(" "));
...
WeChatSearchAutomationController::Result WeChatSearchAutomationController::run(const QStringList& keywords,
  ...
  if (runAdb(adbCatUiDumpArguments(), &output, &error, 8000)) {
    result.uiDumpPreview = output.left(2000);
  }
  result.stage = QStringLiteral("tap_search");
  if (!runAdb(adbTapArguments(options.searchTapX, options.searchTapY), &output, &error)) {
    result.success = false;
    result.message = error;
    return result;
  }
  ...
  for (const QString& keyword : keywords) {
    ...
    if (!runAdb(adbKeyEventArguments(66), &output, &error)) {
      result.success = false;
      result.message = error;
      return result;
    }
    ...
    if (options.resultTapX > 0 && options.resultTapY > 0) {
      ...
      runAdb(adbTapArguments(options.resultTapX, options.resultTapY), &output, &error);
    }
  }
